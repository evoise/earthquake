...
export async function getKandilliArchive(date: string, date_end?: string, limit: number = 100, skip: number = 0): Promise<Earthquake[]> {
  try {
    const params: any = {
      date,
      limit,
    ...
    const response = await axios.get('https://api.orhanaydogdu.com.tr/deprem/kandilli/archive', {
      params,
      timeout: 30000,
      headers: {
    ...
    if (data && data.status && data.result && Array.isArray(data.result)) {
      data.result.forEach((item: any) => {
        if (item.geojson && item.geojson.coordinates && item.mag !== undefined) {
          ...
          earthquakes.push({
            id: `kandilli-${item.earthquake_id || Date.now()}-${Math.random()}`,
